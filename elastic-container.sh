...
set_fleet_values() {
  fingerprint=$(${COMPOSE} exec -w /usr/share/elasticsearch/config/certs/ca es01 cat ca.crt | openssl x509 -noout -fingerprint -sha256 | cut -d "=" -f 2 | tr -d :)
  printf '{"fleet_server_hosts": ["%s"]}' "https://${servername}:${FLEET_PORT}" | curl -k --silent --user "${ELASTIC_USERNAME}:${ELASTIC_PASSWORD}" -XPUT "${HEADERS[@]}" "${LOCAL_KBN_URL}/api/fleet/settings" -d @- | jq
  printf '{"hosts": ["%s"]}' "https://${servername}:9200" | curl -k --silent --user "${ELASTIC_USERNAME}:${ELASTIC_PASSWORD}" -XPUT "${HEADERS[@]}" "${LOCAL_KBN_URL}/api/fleet/outputs/fleet-default-output" -d @- | jq
...
"start")
  ...
  echo "READY SET GO!"
  echo
  echo "Browse to https://${servername}:${KIBANA_PORT}"
  echo
  ;;
...
"restart")
  ...
  echo "#####"
  ${COMPOSE} restart es01 es02 es03 kibana fleet-server
  ;;
